...
to integrate directories of files (notes), especially when coming from multiple
sources, and to provide easy narrowing via =consult=. But notes can be in principle
added from any source that can be integrated with consult.
