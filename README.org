...
Active development, issues, and releases are now at https://codeberg.org/mclear-tools/consult-notes
...
For precise column alignment, set =consult-notes-denote-title-width= to a fixed
number of columns. Titles longer than this value are truncated with an ellipsis,
and shorter titles are padded, so that keywords and other fields start at a
consistent column:

#+begin_src emacs-lisp
(setq consult-notes-denote-title-width 40)
#+end_src

When =consult-notes-denote-title-width= is nil (the default), the title column
auto-sizes based on the widest title plus =consult-notes-denote-title-margin=.
...
If you use =denote-silo-extras= and want each silo to appear as a separate
source in =consult-notes=, see [[https://github.com/mclear-tools/consult-notes/issues/63][issue #63]] for a working configuration example
that creates per-silo sources with their own narrowing keys.
...
When you select a node, =consult-notes= opens it using
=consult-notes-org-roam-open-function=, which defaults to =org-roam-node-visit=.
This opens the node in the current window and respects =other-window-prefix=
(=C-x 4 4=). To restore the previous behavior (which follows
=org-link-frame-setup= and typically opens in another window), set:

#+begin_src emacs-lisp
(setq consult-notes-org-roam-open-function #'org-roam-node-open)
#+end_src

If your input does not match an existing node, =consult-notes= will create a new
org-roam node using =org-roam-capture-=. This mirrors the behavior of
=org-roam-node-find= and the denote integration.
